...
test("the lag alarm's real trip point survives bucket quantisation", () => {
  // 延迟只能按整桶观测：日志读到的是「最新一个完整的桶结束了多久」，取值是
  // `抖动 + 300×k`（k 为落后的桶数），而且总不小于真实延迟。所以门槛真正的跳变点，是它
  // 下面最近的那一档 —— 门槛 720 秒意味着「真实延迟超过 600 秒就告警」，而不是超过 720。
  //
  // 抖动来自 cron：`controller.scheduledTime` 不落在整分钟上（生产实测 11–55 秒）。它
  // **不影响这个跳变点**，因为抖动恒小于一个粒度：k=2 时观测值最大 659 秒仍不告警，
  // k=3 时至少 900 秒必定告警。下面按 k 的整数倍推导即可，不必把抖动算进去。
  //
