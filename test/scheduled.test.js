 ...
 *
 * 于是可观测的延迟被量化到 300 秒一档，而且总是**不小于**真实延迟：真实延迟 720 秒时，
 * 最新一个完整的桶结束在 900 秒之前，日志里读到的就是 900 秒。这个方向是保守的 ——
 * 告警只会更早，不会更晚。
 *
 * 这里的时刻一律取整分钟，所以量化后的档位就是 0 / 300 / 600 …。**生产不是这样**：cron
 * 的 `scheduledTime` 带 11–55 秒抖动（实测），档位相应变成「抖动 + 300×k」。抖动恒小于
 * 一个粒度，不改变任何跳变点，所以这里不建模它 —— 但别把「档位从 0 起」当成生产的形态。
...
test("the rate denominator follows the seconds actually covered, not the bucket count", async () => {
  ...
  // 看门狗放行的方向。
  //
  // **这不是边缘情况，而是生产的常态。** cron 的 `controller.scheduledTime` 并不落在整分钟
  // 上：生产实测 98 个样本的 `mod 600` 取 11 / 14 / 30 / 47 / 55 秒，无一为 0。也就是说
  // 「桶数 × 粒度」这个分母在生产里**每一轮都是错的**，按实际覆盖秒数算才是天天在跑的
  // 那条路径。（`controller.scheduledTime` 缺席退回 `Date.now()` 时同理。）
