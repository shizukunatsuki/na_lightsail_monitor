...
一个 Cloudflare Worker，每 2 分钟查一次指定 AWS Lightsail 实例本月已用的流量。两道线任意一道被触碰就把实例停掉：月度总量达到套餐额度的设定比例（默认 80%），或者按最近一小时的速率算、剩余额度撑不过一个反应视野。下个月额度重置后再自动启回来。它防的是 Lightsail 的流量超额账单——那种没有上限、按 GB 计费、要到月底账单上才现身的东西。
...
正常日子的基线是 4 次调用：两个指标的月度查询，加上突发闸门那一小时窗口的两次查询。再往上第 5 次是 `GetInstanceState`，第 6 次才是真正的动作。总量已经越过静态线时会跳过突发闸门——已经确认要停，没必要再花两次调用去问速率。

**突发闸门。** 月初至今是个**滞后的总量**，它看不出「最近一小时正在以 2 Gbps 烧」。默认的 20% 余量在 1 Gbps 下只够烧 29 分钟、2 Gbps 下 15 分钟——并不比一个检测回路长多少。所以除了静态线，每次触发还额外查最近一小时（`period: 300`），算出当前速率，然后问一个动态问题：
...
速率的分母取「实际落库的数据点个数 × 300 秒」，而不是窗口长度。指标有几分钟延迟，一小时的窗口尾部通常还是空的；拿整个 3600 秒去除会把速率算低——而算低正是不安全的那个方向。
...
- **月度查询的 `period` 取 86400（一天）而不是 3600（一小时）。** 免费版每次调用的 CPU 时间是 10 ms。按天粒度，「月初至今」每个指标返回约 31 个数据点；按小时会返回约 744 个，光解析那份 JSON 就会撑爆 CPU 预算。突发闸门那一小时的窗口用 `period: 300`，每个指标最多 13 个点，解析成本可以忽略——正常触发一共解析四份响应，仍然远在预算之内。
...
打桩按 `period` 区分月度查询和突发窗口，两者能分别给量。这一点是必需的：否则「本月用了 800 GiB」会被当成「最近一小时烧了 800 GiB」，突发闸门在每个用例里都会误触发。
