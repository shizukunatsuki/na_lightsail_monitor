...
1. **用量** —— 把月初至今的 `NetworkIn` + `NetworkOut` 求和，再除以 2³⁰。固定两次 API
   调用。
2. **达到或超过 `QUOTA_GIB × THRESHOLD`** → 查询实例状态。`running` → 停机并按 error
...
### 单位：`QUOTA_GIB` 是 GiB，`THRESHOLD` 是唯一的裕度

**`QUOTA_GIB` 以 2³⁰ 字节为一个单位**，Worker 也是把原始字节数除以 2³⁰ 之后再比较。这个
单位是特意选的，目的是让你可以把 Lightsail `GetBundles` 返回的 `transferPerMonthInGb`
**原样填进去，不做任何换算**：
...
两条来自同一份 `GetBundles` 响应的线索：AWS 自己把「1 TB」记作 **1024** GB，说明这些前缀是
按二进制缩放的；同一个套餐里还有 `ramSizeInGb: 0.5`，而 0.5 GB 的内存只可能是 512 MiB
（2²⁹ 字节）。两处放在一起，指向这套 `InGb` 字段用的就是 GiB。

> **这是基于字段命名一致性的推断，不是 AWS 的明文定义。** AWS 从未在文档里声明
> `transferPerMonthInGb` 的字节含义。下面这一节说明的就是：即便这个推断是错的，默认配置
> 依然安全。

#### 两种解释下都不会超额

默认配置（`QUOTA_GIB = 1024`、`THRESHOLD = 0.8`）的停机线是 **819.2 GiB**，也就是约
**879.6 × 10⁹ 字节**。它占真实额度的比例取决于 AWS 的「1 TB」究竟是多少字节：

| 若 Lightsail 的 1 TB 是… | 879.6 × 10⁹ 字节相当于 |
| --- | --- |
| 2⁴⁰ 字节（即上面推断的 1024 GiB） | 额度的 **80%** |
| 1024 × 10⁹ 字节（前缀按二进制、单位按十进制） | 额度的 **约 85.9%** |

两种解释下都没有超额，所以这个取值在任一定义下都是安全的。

#### 裕度只剩一层了

此前的版本把字节按 10⁹ 换算，同时又把额度保守地写成 1000，等于叠了两层裕度 —— 实际停机点
落在真实额度的约 **72.8%**，比配置上写的 80% 低得多。现在单位对齐了，那一层隐性裕度消失，
停机点上移到 **80%**，也就是 `THRESHOLD` 字面上的意思。

想要更大的余量，就直接调低 `THRESHOLD`（比如 `0.7`），**不要再靠单位错配来制造缓冲** ——
那种缓冲量既写不进配置，也没法在读代码时看出来。
...
| `QUOTA_GIB` | `1024` | 套餐额度，单位 GiB（2³⁰ 字节）；直接填 `transferPerMonthInGb` |
...
永远够不到的 81,920 GiB 上限。同理，`QUOTA_GIB` 若不能解析为正数就是硬错误，因为与 `NaN`
的比较会被读作「已超额」并停掉实例。

`QUOTA_GIB` **没有向后兼容**：从旧版本升级上来时，如果线上还留着 `QUOTA_GB` 这个绑定，
Worker 会在第一次触发时因缺少 `QUOTA_GIB` 而报错。这是刻意的 —— 一个静默的默认值会让你
在毫不知情的情况下按错误的额度运行。

你的套餐额度可以在 *Lightsail → 实例 → 你的实例 → 网络 → 每月数据传输* 里找到，或者用
`aws lightsail get-bundles` 直接读 `transferPerMonthInGb`。

### 3. 告警靠 AWS Budgets，它是唯一的信号源

这个 Worker 不会主动通知任何人。停机时它往 Workers 日志里写一行 `console.error`；任何
失败都以异常的形式向上抛出，由 Workers 把那次调用记为失败。这两件事都只留在 Workers 日志
里，而日志是个只能主动去拉的次要信号 —— 要看见它们，你得自己打开 dashboard 或者跑一次
`npx wrangler tail`。没有人会主动去翻日志，实例下线这件事你多半是在自己打开站点时才发现的。
...
这条通道是单向的：日志只会待在那里等你去看，不会来找你。唯一会主动送达的告警是上面
第 3 节的 AWS Budgets。
...
- **指标有延迟。** Lightsail 的指标数据比真实流量晚几分钟落库，所以观察到的数字总是落后于
  实际用量。默认配置在 1 TB 套餐上留了 204.8 GiB 的余量，这很充裕；把 `THRESHOLD` 往 1.0
  调会侵蚀这块余量，而现在它是唯一的一层裕度 —— 详见上文的单位一节。
